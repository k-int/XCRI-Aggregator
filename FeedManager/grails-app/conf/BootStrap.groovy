...
class BootStrap {
    ...
    def init = { servletContext ->
      ...
      if ( ApplicationHolder.application.config.feedmanager?.default?.aggr != null ) {
        ...
        def default_aggr = AggregationService.findByBaseurl(ApplicationHolder.application.config.feedmanager.default.aggr)
        if ( default_aggr == null ) {
          default_aggr = new AggregationService(baseurl:ApplicationHolder.application.config.feedmanager.default.aggr,
                                                identity:'admin',
                                                credentials:'password',
                                                owner:user);
  
          if(default_aggr.save(flush: true)) {
        ...
        validate(feeds,user,default_aggr);
      }
      else {
