...
class FeedRunnerService {
  ...
  def doCollectFeed(feed_definition) {
    ...
    try {
      ...
      if ( feed_definition instanceof SingleFileDatafeed ) {
        ...
        uploadStream(resource.openStream(),aggregator_service,feed_definition.dataProvider);
      }
    }
    catch ( Exception e ) {
  ...
  def uploadStream(input_stream,target_service,dataProvider) {
    ...
    try {
      ...
      target_service.request(POST) {request ->
        ...
        multipart_entity.addPart( "owner", new StringBody( dataProvider, "text/plain", Charset.forName( "UTF-8" )))  // Owner
        ...
        response.failure = { resp ->
          ...
          feed_definition.status=4
          feed_definition.statusMessage="Error uploading to aggregator. ${resp.status}"
